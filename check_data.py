...
CHECKS = [
    # (subdir, archivo_db, tabla, col_serie)
...
current_dir = None
for subdir, filename, table, col_serie in CHECKS:
    ...
    if not path.exists():
        print(f"  {FAIL}  {filename:<45} NO ENCONTRADO")
        total_fail += 1
        continue
    ...
    if filename.endswith(".json"):
        print(f"  {OK}  {filename:<45} {size_mb:.1f} MB")
        total_ok += 1
        continue

    try:
        conn = sqlite3.connect(str(path))
        series = conn.execute(f"SELECT COUNT(DISTINCT {col_serie}) FROM {table}").fetchone()[0]
        rows   = conn.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0]
        conn.close()
        print(f"  {OK}  {filename:<45} {size_mb:>5.1f} MB  {series:>5} series  {rows:>8,} filas")
        total_ok += 1
    except Exception as e:
        print(f"  {WARN}  {filename:<45} {size_mb:.1f} MB  ERROR: {e}")
        total_fail += 1
...
if total_fail > 0:
    ...
    print("  bash sync_data.sh TU_USUARIO_GITHUB\n")
